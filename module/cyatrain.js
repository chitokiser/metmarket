 // testnet
 let contractAddress = {
    cyatrainAddr: "0x6686C2298A8cDe6eACC975da5E2316641f934854",
   ...
   let contractAbi = {
    ...
    cyatrain: [
      ...
      "function g4(address _user) public view returns(uint depo,uint mynum,uint mytn)",
  ...
  const topDataSync = async () => {
    ...
    const cyatrainContract = new ethers.Contract(contractAddress.cyatrainAddr,contractAbi.cyatrain,provider);
    
    const tbal = await cyatrainContract.g1();
    
    const creatnum = await cyatrainContract.pllength();
    //계약잔고
    const fcyabal = await cyatrainContract.g1();
   
    document.getElementById("farmtotal").innerHTML = (tbal/1e18);
