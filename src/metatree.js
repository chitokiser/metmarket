 // metatree
 let mtAddress = {
    metatreeAddr: "0x567B36EbE4c9cdDeD1a2F3192A41a96b4e342974",
  };  
   let mtAbi = {
    ...
    metatree: [
      ...
      "function myinput(address user) public view returns(uint256,uint256,uint256,bool)",
  ...
  let MemberLogin = async () => {
    ...
    let myt =  parseInt (await mybond[1]);
    let mytiket =  parseInt (await mybond[2]);
    let complete = await mybond[3] === 'true';
    document.getElementById("Mdepo").innerHTML= (mydepo);  //충전금 총액 
    document.getElementById("Bar").style.width = `${mytiket/10*100}%`; // CHECK:: 소수점으로 나오는 것 같아 *100 했습니다.  
    let time2 = parseInt(604800);  // 1주일
    let nowt = Math.floor(new Date().getTime() / 1000);
    let left = parseInt((myt + time2) - nowt); // 남은 시간 계산
